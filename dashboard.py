...
# Box Plot
    ...
    fig = px.box(df_filtered, x='month', y='export_kwh', points='all', title="Monthly Export Distribution")
    st.plotly_chart(fig)

# Cumulative Plot
elif selected_plot == "Cumulative Export":
    df_filtered['cumulative_export_kwh'] = df_filtered['export_kwh'].cumsum()
    fig = px.line(df_filtered, x='time', y='cumulative_export_kwh', title='Cumulative Energy Export (kWh)', labels={'cumulative_export_kwh': 'Energy Export (kWh)'})
    st.plotly_chart(fig)
